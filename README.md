...
An assortative-mating population simulator (with optional island migration), plus
utilities for finding relative pairs across generations and post-processing
simulation output.
...
| Module | Contents |
|--------|----------|
| `core_simulation.py` | `AssortativeMatingSimulation` — the base simulation engine, plus module-level utility functions |
| `island_migration.py` | `IslandMigrationSimulation` — subclass adding island population structure and phenotype-driven migration |
| `relationship_finder.py` | Pedigree/relationship-pair extraction: `find_relationship_pairs`, `extract_genealogy_info`, and related helpers (set-based merge algorithm) |
| `postprocessing.py` | Measure extraction, relationship-correlation computation, and saving simulation results to disk |
| `tests/test_simulation.py` | Pytest-discoverable validation suite for the engine |
...
10. [relationship_finder.py Reference](#10-relationship_finderpy-reference)
11. [postprocessing.py Reference](#11-postprocessingpy-reference)
...
These are module-level helpers in `core_simulation.py`.
...
```python
from island_migration import IslandMigrationSimulation
```
...
## 10. relationship_finder.py Reference

Finds pairs of individuals related in a specified way (siblings, cousins, grandparent-grandchild,
etc.) by walking maternal/paternal/spouse links through the pedigree recorded in a simulation's
`results` dict, using a set-based `pandas.merge` approach for performance. (An older
multiprocessing graph-traversal implementation is archived at
`Scripts/Archive/SimulationFunctions_legacy/find_relative.py` and is no longer used.)

```python
from relationship_finder import find_relationship_pairs, extract_genealogy_info
```

### `extract_genealogy_info(results, generations=None) → pd.DataFrame`
Builds a `[ID, Father.ID, Mother.ID, Spouse.ID, Generation]` table from simulation history. Most
other functions in this module consume this table's structure internally, but you generally won't
need to call it directly — `find_relationship_pairs` does so for you.

### `get_genealogy_summary(genealogy_df) → pd.DataFrame`
Per-generation summary stats (individual counts, mating rates) from a genealogy table.

### `find_relationship_pairs(results, relationship_path, generations=None, output_format='long', use_sparse=None, n_jobs=None) → pd.DataFrame or scipy.sparse matrix`
The main entry point. `relationship_path` is a code string built from `M` (mother), `P` (father),
`C` (child), `S` (spouse) — e.g. `"S"` for siblings (shared parents), `"MC"` for maternal cousins.
`output_format='long'` returns a DataFrame of `Person_ID`/`Relative_ID` pairs; `'sparse'` returns a
sparse relationship matrix instead (better for very large populations).

```python
siblings = find_relationship_pairs(results, "S", output_format='long')
```

### `save_sparse_relationship_matrix` / `load_sparse_relationship_matrix` / `sparse_matrix_to_long_format`
Persistence helpers for sparse relationship matrices produced with `output_format='sparse'`.

### `get_matrix_memory_info(results, generations=None) → dict`
Estimates memory requirements for a dense vs. sparse relationship matrix at the given population
size, useful for deciding which `output_format` to use before running a large query.

---

## 11. postprocessing.py Reference

Extracts individual- or pair-level measures from simulation output, computes correlations by
relationship type, and saves simulation results to disk. Combines what were previously two
separate modules (`extract_measures.py` and `save_simulation_data.py`).

```python
from postprocessing import (
    extract_individual_measures, extract_measures_for_pairs,
    compute_correlations_for_multiple_variables, save_simulation_results,
)
```

### `extract_individual_measures(results, variable_names, generations=None) → pd.DataFrame`
Pulls individual-level variables (e.g. `Y1`, `TPO1`) from `results['HISTORY']['PHEN']` into a
long `[Generation, ID, ...variables]` DataFrame.

### `extract_measures_for_pairs(results, pairs_df, variable_names, id_col1='Person_ID', id_col2='Relative_ID') → pd.DataFrame`
Joins individual measures onto a relationship-pairs table (e.g. the output of
`relationship_finder.find_relationship_pairs`), producing `{var}_1`/`{var}_2` columns per pair.

```python
siblings = find_relationship_pairs(results, "S", output_format='long')
siblings_scores = extract_measures_for_pairs(results, siblings, ['Y1', 'TPO1'])
```

### `compute_correlation_by_relationship(pairs_with_measures, variable_name, relationship_col='Relationship') → pd.DataFrame`
Pearson correlation (and covariance, N, p-value) for one variable, grouped by relationship type.

### `compute_correlations_for_multiple_variables(pairs_with_measures, variable_names, relationship_col='Relationship') → pd.DataFrame`
Same as above, across several variables at once — the function most callers actually use.

### `save_measures_to_file(measures_df, output_path, include_index=False)`
Writes a measures/correlations DataFrame to CSV or TSV, chosen by the `output_path` extension.

### `save_simulation_results(results, output_folder, file_prefix="sim_output", scope="final")`
Saves a simulation's `results` dict to TSV files: summary/covariance logs always, plus
`PHEN`/`XO`/`XL`/mates data for `scope="final"` (last generation only), `"all"` (every recorded
generation), or a specific generation index/list of indices.
...
```bash
cd Scripts/SimulationFunctions
python -m pytest tests/test_simulation.py -v
# or:
python tests/test_simulation.py
```

Six tests cover: baseline AM (no islands/mtDNA), mtDNA maternal transmission, island migration and
stratification, islands + mtDNA combined, burn-in behavior, and seed reproducibility. See
`tests/test_simulation.py`'s module docstring for details on each.
