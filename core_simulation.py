...
class AssortativeMatingSimulation: 
    ...
    def __init__(self, 
        ...
        if mt_mat is None:
            self.mt_mat = np.zeros((2, 2))
        else:
            mt_arr = np.array(mt_mat, dtype=float)
            if not np.allclose(mt_arr - np.diag(np.diag(mt_arr)), 0):
                raise ValueError("mt_mat must be diagonal — mtDNA has no cross-trait effects.")
            self.mt_mat = mt_arr
    ...
    def _reproduce(self, mates_dict, xo_parent_gen_full, xl_parent_gen_full, phendata_parent_gen_df_full):
        ...
        mt_y = mothers[['Y1','Y2']].values @ self.mt_mat.T
        e_new = np.random.multivariate_normal(np.zeros(2), self.cove_mat, size=total)
        
        # Y = AO + AL + F + MT + E
        y_new = ao_new_raw + al_new_raw + f_y + mt_y + e_new
        ...
        new_df[['F1','F2']] = f_y; new_df[['MT1','MT2']] = mt_y
        new_df[['E1','E2']] = e_new; new_df[['Y1','Y2']] = y_new
    ...
    def _initialize_generation_zero(self):
        ...
        self.phen_df[['F1','F2']] = f_gen0; self.phen_df[['MT1','MT2']] = mt_gen0
        self.phen_df[['E1','E2']] = e_gen0; self.phen_df[['Y1','Y2']] = y_gen0
        ...
        self._extend_phen_column_names()
        print("Generation 0 initialized.")

    # ---------------------------------------------------------------------------
    # Extension hooks — no-ops in base class, override in subclasses
    # ---------------------------------------------------------------------------

    def _extend_phen_column_names(self):
        """Called once at the end of _initialize_generation_zero.
        Subclasses append extra column names to self.phen_column_names and
        add the corresponding columns to self.phen_df here."""
        pass

    def _pre_mating_hook(self, gen_abs_idx: int):
        """Called before _assort_mate each generation (burn-in and main).
        gen_abs_idx is 0-based absolute generation index across burn-in + main.
        No-op in base class."""
        pass

    def _post_offspring_hook(self, offspring_data: dict, gen_abs_idx: int) -> dict:
        """Called after _reproduce each generation.
        Receives offspring_data dict and must return it (possibly modified).
        gen_abs_idx is 0-based absolute generation index across burn-in + main.
        No-op in base class."""
        return offspring_data

    # ---------------------------------------------------------------------------
    # Generation runner — contains the loop body shared by burn-in and main loop
    # ---------------------------------------------------------------------------

    def _run_generation(self, gen_abs_idx: int, am_idx: int, pop_size_target: int, save: bool):
        """Execute one generation: migrate (hook) -> mate -> reproduce -> (optionally) record.

        Parameters
        ----------
        gen_abs_idx : int
            0-based absolute generation counter across burn-in + main phases.
        am_idx : int
            Index into self.am_list to pick the mating correlation for this generation.
        pop_size_target : int
            Target offspring population size.
        save : bool
            Whether to record summary stats, history, and write the summary file.
        """
        display_num = gen_abs_idx + 1
        print(f"\n--- Generation {display_num} (abs={gen_abs_idx}, {'MAIN' if save else 'BURN-IN'}) ---")

        mate_cor_mu = self.am_list[am_idx]

        self._pre_mating_hook(gen_abs_idx)

        print(f"Generation {display_num}: Performing '{self.mating_type}' assortative mating...")
        mates = self._assort_mate(self.phen_df, self.mating_type, mate_cor_mu, pop_size_target)
        if len(mates['males.PHENDATA']) == 0:
            print(f"Generation {display_num}: No pairs formed — stopping.")
            return False

        print(f"Generation {display_num}: Simulating reproduction...")
        offspring_data = self._reproduce(mates, self.xo, self.xl, self.phen_df)
        if offspring_data['PHEN'].empty:
            print(f"Generation {display_num}: Empty offspring — stopping.")
            return False

        offspring_data = self._post_offspring_hook(offspring_data, gen_abs_idx)

        self.xo = offspring_data['XO']
        self.xl = offspring_data['XL']
        self.phen_df = offspring_data['PHEN']

        if not save:
            return True

        print(f"Generation {display_num}: Calculating summary statistics...")
        ...
        if len(valid_cols) > 1:
            full_cov_df = temp_phen_df_for_cov[valid_cols].cov()

        nan_2x2 = np.full((2, 2), np.nan)
        hapsO_covs_dict = np.nan
        if not full_cov_df.empty:
            o_cols = ['TPO1','TMO1','NTPO1','NTMO1','TPO2','TMO2','NTPO2','NTMO2']
            if all(c in full_cov_df.index for c in o_cols):
                hapsO_covs_dict = full_cov_df.loc[o_cols, o_cols].to_dict()

        # am_list look-ahead for next gen (used in summary)
        next_am_idx = am_idx + 1
        if next_am_idx < len(self.am_list):
            mc = self.am_list[next_am_idx]
        else:
            mc = self.am_list[-1]
        mc = mc.tolist() if hasattr(mc, 'tolist') else mc

        r_gen_num = gen_abs_idx - self.n_burn_in + 1  # 1-based main-phase generation number
        summary_this_gen = {
            'GEN': r_gen_num, 'GEN_ABS': gen_abs_idx,
            'NUM.CVs': self.num_cvs, 'MATE.COR': mc,
            'POPSIZE': len(self.phen_df)
        }
        for comp_name, cols in {
            "VAO": ["AO1", "AO2"], "VAL": ["AL1", "AL2"],
            "VF":  ["F1",  "F2"],  "VMT": ["MT1", "MT2"],
            "VE":  ["E1",  "E2"],  "VP":  ["Y1",  "Y2"]
        }.items():
            if all(c in self.phen_df.columns for c in cols):
                cov_data = self.phen_df[cols].dropna().values
                summary_this_gen[comp_name] = np.cov(cov_data, rowvar=False).tolist() if len(cov_data) > 1 else nan_2x2.tolist()
            else:
                summary_this_gen[comp_name] = nan_2x2.tolist()
        ...
        vmt_diag = np.diag(np.array(summary_this_gen.get('VMT', [[np.nan, np.nan]])))
        ...
        print(f"--- Generation {display_num} Processing Done ---")
    ...
    def _write_simulation_summary_to_file(self):
        ...
        try:
            with open(self.output_summary_filename, 'w') as f:
                ...
                f.write(f"mt_mat: {self._format_matrix_for_file(self.mt_mat)}\n")
    ...
    def run_simulation(self):
        print(f"Starting simulation: {self.n_burn_in} burn-in + {self.num_generations} main generations, "
              f"'{self.mating_type}' assortment.")
        if self.phen_df is None:
            return None

        # --- Burn-in phase (no history/summary recording) ---
        if self.n_burn_in > 0:
            print(f"\n=== Burn-in phase: {self.n_burn_in} generations ===")
            burn_am_idx = 0  # use am_list[0] for the whole burn-in
            burn_pop = self.pop_vector[0]
            for bi in range(self.n_burn_in):
                ok = self._run_generation(
                    gen_abs_idx=bi,
                    am_idx=burn_am_idx,
                    pop_size_target=burn_pop,
                    save=False
                )
                if not ok:
                    break
        ...
        print(f"\n=== Main simulation phase: {self.num_generations} generations ===")
        for cur_gen_py_idx in range(self.num_generations):
            gen_abs_idx = self.n_burn_in + cur_gen_py_idx
            ok = self._run_generation(
                gen_abs_idx=gen_abs_idx,
                am_idx=cur_gen_py_idx,
                pop_size_target=self.pop_vector[cur_gen_py_idx],
                save=True
            )
            if not ok:
                break
